...
[下载 OpenClaw 版技能包](/signal-skills/download) 并解压到任意目录，例如：

```bash
~/.agents/skills/trade-signal-skills-openclaw/
```

然后编辑 `~/.openclaw/openclaw.json`，配置 Skills 加载路径：
...
复制技能包内的 `mcporter.json.example` 为 `~/.mcporter/mcporter.json`，填入实际的 Feed 服务地址和 API Key：
